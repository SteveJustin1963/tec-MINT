...
## Arithmetic (these will set /c and /r flags)
  ...
  state.dict("/deg") = @(s) deg_to_rad(s);
  state.dict("/rad") = @(s) rad_to_deg(s);
...
## ============================================================================
## START INSERTING ALL TRIGONOMETRIC FUNCTIONS HERE (STEP 2)
## ============================================================================
...
function s = deg_to_rad(s)
  ...
  s = push(s, radians);
  if state.debug
    debug_after_op("/deg", sprintf("%g degrees = %g radians", degrees, radians), s);
  endif
endfunction
...
function s = rad_to_deg(s)
  ...
  s = push(s, degrees);
  if state.debug
    debug_after_op("/rad", sprintf("%g radians = %g degrees", radians, degrees), s);
  endif
endfunction
...
if ch == '/' && i < length(line)
  ...
  if i <= length(line)-4
    ...
    if any(strcmp(five_char, {'/sinh', '/cosh', '/tanh', '/asin', '/acos', '/atan', '/sqrt'}))
      if !isempty(current_token)
        tokens{end+1} = strtrim(current_token);
        current_token = "";
      endif
  ...
  if i <= length(line)-3
    ...
    if any(strcmp(four_char, {'/sin', '/cos', '/tan', '/abs', '/exp', '/log', '/deg', '/rad'}))
      if !isempty(current_token)
        tokens{end+1} = strtrim(current_token);
        current_token = "";
      endif
...
function s = show_help(s)
  ...
  printf("CONVERT   | * /deg | convert degrees to radians                | n -- n       | DONE\n");
  printf("CONVERT   | * /rad | convert radians to degrees                | n -- n       | DONE\n");
