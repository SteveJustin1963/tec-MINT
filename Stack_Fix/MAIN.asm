...
start:
    ; ld SP,DSTACK		; start of MINT, OLD - this causes the collision!
    LD   SP,CPUSTACK  ; NEW - CPU stack at top of RAM
    call init		    ; setups
...
ALLOC_:      
   POP   de                    ; requested size
   LD    hl,(vHeapPtr)        
   PUSH  hl                   ; return current heap ptr
   ADD   hl,de                ; calculate new heap ptr
   
   ; ADD STACK COLLISION PROTECTION:
   LD    bc,STACKGUARD        ; Load guard boundary
   OR    a                    ; Clear carry
   SBC   hl,bc               ; Check if heap would hit stack area
   JP    NC,alloc_overflow    ; Abort if collision
   ADD   hl,bc               ; Restore hl to new heap ptr
   
   LD    (vHeapPtr),hl       ; Update heap pointer
   JP    (iy)                ; Continue

alloc_overflow:
   ; Handle heap overflow - for now just return original pointer
   ADD   hl,bc               ; Restore hl 
   JP    (iy)                ; Return without allocating
