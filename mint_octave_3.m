...
## Version 2.5.x - Multi-line function definition support
## ----------------------------------------------------------------------
function mint_octave()
  ...
  ## Interpreter state
  state.stack = [];
  state.vars = containers.Map();  ## Use Map instead of array
  ...
  printf("MINT-Octave REPL v2.5.2 (2025-10-01). Type 'bye' to quit.\n");
...
if !state.capture_mode
  ...
  ## Look for :XXX (1-3 uppercase letters) at start of line
  if length(trimmed) >= 2 && trimmed(1) == ':'
    ## Extract function name
    end_idx = 2;
    while end_idx <= length(trimmed) && trimmed(end_idx) >= 'A' && trimmed(end_idx) <= 'Z'
      end_idx++;
    endwhile
    func_name = trimmed(2:end_idx-1);
    
    ## Validate: 1-3 uppercase letters
    if length(func_name) >= 1 && length(func_name) <= 3
      ## Entering capture mode
      state.capture_mode = true;
      state.capture_name = func_name;
      state.capture_buffer = line;
        
        if state.debug
          printf("[DEBUG] >>> ENTERING CAPTURE MODE for function '%s'\n", state.capture_name);
        endif
        ...
        if any(strfind(line, ';'))
          ...
          try
            if state.debug
              printf("[DEBUG] SINGLE-LINE FUNCTION: %s\n", state.capture_buffer);
            endif
            interpret_line(state.capture_buffer);
            if state.debug
              printf("[DEBUG] <<< FUNCTION '%s' SAVED\n", state.capture_name);
            endif
          catch err
            fprintf("ERROR: %s\n", err.message);
            if state.debug
    ...
    if state.capture_mode
      ...
      if state.debug
        printf("[DEBUG] CAPTURE: accumulating line into buffer\n");
      endif
      ...
      if any(strfind(line, ';'))
        ...
        if state.debug
          printf("[DEBUG] <<< EXITING CAPTURE MODE\n");
          printf("[DEBUG] COMPLETE BUFFER: %s\n", state.capture_buffer);
        endif
        ...
        try
          interpret_line(state.capture_buffer);
          if state.debug
            printf("[DEBUG] FUNCTION '%s' SAVED SUCCESSFULLY\n", state.capture_name);
...
function s = store_var(s)
  global state;
  if isempty(state.last_var) || (isnumeric(state.last_var) && state.last_var < 1)
    error("! requires a variable before it (e.g., 10 x !)");
  endif
  [s, val] = pop(s);
  var_name = state.last_var;
  old_val = 0;
  if isKey(state.vars, var_name)
    old_val = state.vars(var_name);
  endif
  if state.debug
    printf("[DEBUG] STORE: %s = %g (was %g)\n", var_name, val, old_val);
  endif
  ...
  while i <= length(tokens)
    ...
    if strcmp(tok, "[")
      ...
      while i <= length(tokens) && !strcmp(tokens{i}, "]")
        elem_tok = tokens{i};
        ## Parse the element
        if length(elem_tok) >= 1 && length(elem_tok) <= 3 && all(elem_tok >= 'a' & elem_tok <= 'z')
  
  ## Variable reference
  if isKey(state.vars, elem_tok)
    array_elements(end+1) = state.vars(elem_tok);
  else
    array_elements(end+1) = 0;  ## Uninitialized
  endif
...
if length(tok) >= 2 && length(tok) <= 4 && tok(1) == ':'
  func_name = tok(2:end);
  ## Validate: 1-3 uppercase letters
  if length(func_name) >= 1 && length(func_name) <= 3 && all(func_name >= 'A' & func_name <= 'Z')
    compile_mode = true;
    current_def = {};
    current_def{1} = func_name;
    if state.debug
    ...
    error("Invalid function name: %s (must be 1-3 uppercase letters)", func_name);
  endif
    ...
    elseif strcmp(tok, ";")
      if !compile_mode
        ...
        if state.debug
          printf("[DEBUG] WARNING: Ignoring standalone ';' outside function definition\n");
        endif
        i++;
        continue;
      endif
...
elseif length(tok) >= 1 && length(tok) <= 3 && all(tok >= 'a' & tok <= 'z') && i < length(tokens) && strcmp(tokens{i+1}, "!")
  ## This is a store operation - save variable name
  state.last_var = tok;
  if state.debug
    printf("[DEBUG] STORE PREP: variable '%s' ready for !\n", tok);
  endif
  i++;
...
if ch == '/' && i < length(line)
  ...
  ## Special check for /CS (two-letter operator)
  if next_ch == 'C' && i+1 < length(line) && line(i+2) == 'S'
    if !isempty(current_token)
      tokens{end+1} = strtrim(current_token);
      current_token = "";
    endif
    tokens{end+1} = '/CS';
    i = i + 3;  ## Skip all 3 characters: /, C, S
    continue;
  endif
  
  ## Normal single-char operators
...
## Check if it's a function call (1-3 uppercase letters)
if length(tok) >= 1 && length(tok) <= 3 && all(tok >= 'A' & tok <= 'Z')
  ...
  elseif length(tok) >= 1 && length(tok) <= 3 && all(tok >= 'a' & tok <= 'z')
  
  ## Variable name (1-3 lowercase letters)
  state.last_var = tok;
  if isKey(state.vars, tok)
    val = state.vars(tok);
  else
    val = 0;  ## Uninitialized variables default to 0
  endif
  if state.debug
...
function s = clear_stack(s)
  ...
  depth = length(s);
  if state.debug
    printf("[DEBUG] /CS: clearing stack (had %d items)\n", depth);
  endif
  s = [];  ## Empty the stack
endfunction
...
## --------------------------
## Help Function - UPDATED for 3-character support
## Replace the entire show_help() function with this
## --------------------------
function s = show_help(s)
  printf("\n");
  printf("===================================================================================================\n");
  printf("MINT-Octave Operator Reference (* = implemented)\n");
  printf("Version 2.5.2 - Extended variable and function names (1-3 characters)\n");
  printf("===================================================================================================\n\n");
  ...
  printf("NAMING CONVENTIONS (EXTENDED):\n");
  printf("  Variables:    1-3 lowercase letters  (a, ab, xyz, sum, tmp, arr, cnt, etc.)\n");
  printf("  Functions:    1-3 uppercase letters  (A, AB, XYZ, SUM, AVG, MAX, MIN, etc.)\n");
  printf("  Examples:     100 cnt!  :SUM x y + ;  [arr sum tmp]\n\n");

  printf("===================================================================================================\n");
  printf("Category  | Symbol    | Description                               | Effect       | Status\n");
  printf("--------- | --------- | ----------------------------------------- | ------------ | ------\n");
  printf("MATHS     | *  -      | subtraction                               | n n -- n     | DONE\n");
  printf("MATHS     | *  /      | division (stores remainder in /r)         | n n -- n     | DONE\n");
  printf("MATHS     | *  +      | addition                                  | n n -- n     | DONE\n");
  printf("MATHS     | *  *      | multiplication                            | n n -- n     | DONE\n");
  printf("LOGICAL   | *  >      | greater than comparison                   | n n -- b     | DONE\n");
  printf("LOGICAL   | *  <      | less than comparison                      | n n -- b     | DONE\n");
  printf("LOGICAL   | *  =      | equality comparison                       | n n -- b     | DONE\n");
  printf("LOGICAL   | *  &      | bitwise AND                               | n n -- n     | DONE\n");
  printf("LOGICAL   | *  |      | bitwise OR                                | n n -- n     | DONE\n");
  printf("LOGICAL   | *  ^      | bitwise XOR                               | n n -- n     | DONE\n");
  printf("LOGICAL   | *  ~      | bitwise NOT                               | n -- n       | DONE\n");
  printf("LOGICAL   | *  {      | shift left                                | n -- n       | DONE\n");
  printf("LOGICAL   | *  }      | shift right                               | n -- n       | DONE\n");
  printf("STACK     | *  '      | drop top item                             | n --         | DONE\n");
  printf('STACK     | *  "      | duplicate top item                        | n -- n n     | DONE\n');
  printf("STACK     | *  %%     | over - copy 2nd to top                    | m n -- m n m | DONE\n");
  printf("STACK     | *  $      | swap top 2 items                          | m n -- n m   | DONE\n");
  printf("STACK     | *  /D     | stack depth                               | -- n         | DONE\n");
  printf("STACK     | *  /CS    | clear stack (remove all items)            | ... --       | DONE\n");
  printf("I/O       | *  .      | print number as decimal                   | n --         | DONE\n");
  printf("I/O       | *  ,      | print number as hexadecimal               | n --         | DONE\n");
  printf("I/O       | *  `text` | print literal string                      | --           | DONE\n");
  printf("I/O       | *  /C     | print character (ASCII code)              | n --         | DONE\n");
  printf("I/O       | *  /K     | read single character                     | -- n         | DONE\n");
  printf("I/O       | *  /KS    | read string (returns all ASCII + length)  | -- n n... n  | DONE\n");
  printf("I/O       | *  /N     | print newline                             | --           | DONE\n");
  printf("I/O       |    /O     | output to I/O port                        | n p --       | TODO\n");
  printf("I/O       |    /I     | input from I/O port                       | p -- n       | TODO\n");
  printf("FUNCTION  | * :XXX;   | define function (1-3 uppercase letters)   | --           | DONE\n");
  ...
  printf("LOOP      | *  )      | end loop block                            | --           | DONE\n");
  printf("LOOP      | *  /U     | unlimited loop constant (-1)              | -- -1        | DONE\n");
  printf("LOOP      | *  /W     | while - break if false                    | b --         | DONE\n");
  printf("LOOP      | *  /E     | else (in conditionals)                    | --           | DONE\n");
  printf("LOOP      | *  /F     | false constant (0)                        | -- 0         | DONE\n");
  printf("LOOP      | *  /T     | true constant (-1)                        | -- -1        | DONE\n");
  printf("LOOP      | *  /i     | inner loop counter                        | -- n         | DONE\n");
  printf("LOOP      | *  /j     | outer loop counter                        | -- n         | DONE\n");
  printf("VARIABLE  | * a-zzz   | variable (1-3 lowercase letters)          | -- n         | DONE\n");
  printf("VARIABLE  | *  !      | store value to variable                   | n v --       | DONE\n");
  printf("VARIABLE  |   /V      | address of last variable access           | -- a         | TODO\n");
  printf("ARRAY     | *  [      | begin array definition                    | --           | DONE\n");
  printf("ARRAY     | *  ]      | end array definition                      | -- a         | DONE\n");
  printf("ARRAY     | *  ?      | get array item at index                   | a n -- n     | DONE\n");
  printf("ARRAY     | *  ?!     | set array item at index                   | n a n --     | DONE\n");
  printf("ARRAY     | *  /S     | get array size                            | a -- n       | DONE\n");
  printf("ARRAY     |   /A      | allocate heap memory                      | n -- a       | TODO\n");
  printf("BYTE      |    \\     | enter byte mode                           | --           | TODO\n");
  printf("BYTE      |   \\!     | store byte to memory                      | b a --       | TODO\n");
  printf("BYTE      |   \\[     | begin byte array                          | --           | TODO\n");
  printf("BYTE      |   \\?     | get byte from array                       | a n -- b     | TODO\n");
  printf("SYSVAR    | *  /c     | carry flag                                | -- n         | DONE\n");
  printf("SYSVAR    |   /h      | heap pointer                              | -- a         | TODO\n");
  printf("SYSVAR    | *  /r     | remainder/overflow (from / and *)         | -- n         | DONE\n");
  printf("SYSVAR    |   /k      | text input buffer offset                  | -- a         | TODO\n");
  printf("SYSVAR    |   /s      | stack start address                       | -- a         | TODO\n");
  printf("SYSVAR    |   /z      | last defined function name                | -- c         | TODO\n");
  printf("MISC      | *  //     | comment (to end of line)                  | --           | DONE\n");
  printf("MISC      |   /P      | print prompt                              | --           | TODO\n");
  printf("MISC      | * debug   | toggle debug mode on/off                  | --           | DONE\n");
  printf("MISC      | * list    | list all defined functions                | --           | DONE\n");
  printf("MISC      | * help    | show this help                            | --           | DONE\n");
  printf("===================================================================================================\n\n");
  
  printf("USAGE EXAMPLES:\n");
  printf("  Variables:  100 cnt!  50 sum!  [1 2 3] arr!\n");
  printf("  Functions:  :SUM x y + ;  :AVG SUM 2 / ;  :MAX x y > ( x ) /E ( y ) ;\n");
  printf("  Arrays:     5 x! 10 y! [x y 15] dat!  dat 0 ? .\n");
  printf("  Loops:      10 ( /i . )  /U ( /i 5 = /W /i . )\n");
  printf("  Stack:      1 2 3 /CS  1 2 $ . .  5 \" * .\n\n");
  
  printf("COMMANDS:\n");
  printf("  bye         Quit the interpreter\n");
  printf("  debug       Toggle debug mode\n");
  printf("  list        List all defined functions\n");
  printf("  help        Show this help\n");
  printf("  /CS         Clear the stack\n\n");
