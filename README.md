...
As MINT can be tricky to use on its own, I built MINT-Octave — a version that runs inside a Linux terminal, handles heavier math, floating-point operations, transcendental functions, and other advanced stuff. It supports multiple modes besides FP, such as integer mode, and includes a built-in debugger so all you fellow vibe coders can bounce back and forth between MINT-Octave and the real Z80-based MINT to troubleshoot new code fast. Check the manual. Once you’ve got the hang of it, take off the training wheels and get ready to throw that big ol’ rock. Tip, if yout nuts are sore, ripped off or just broken, use some good old POLA.** 
