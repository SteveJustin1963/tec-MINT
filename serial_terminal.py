...
def main():
    ...
    try:
        ...
        if "Permission denied" in str(e) or "Errno 13" in str(e):
            print(f"Permission denied accessing {SERIAL_PORT}")
            print("\nTo fix this, you need to add your user to the 'dialout' group:")
            print(f"  1. Run: sudo usermod -a -G dialout {os.getenv('USER')}")
            print("  2. Then either:")
            print("     - Log out and log back in (permanent fix)")
            print("     - OR run: newgrp dialout (temporary, current session only)")
            print("\nAlternative quick fix (temporary until device unplugged):")
            ...
            try:
                ...
                if dialout_gid in user_groups:
                    print("\nNote: You're already in the 'dialout' group, but you need to log out")
                    print("      and log back in for it to take effect. Or run: newgrp dialout")
            except KeyError:
                pass  # dialout group doesn't exist on this system
        else:
