...
function s = math_div(s)
  ...
  if state.int_mode
    ## MINT 2 INTEGER DIVISION
    quotient = floor(a / b);
    remainder = mod(a, b);
    s=push(s, quotient);
    state.sys_c = 0;
    state.sys_r = remainder;
    
    if state.debug
      debug_after_op("/", sprintf("%g / %g = %g (remainder=%g)", a, b, quotient, remainder), s);
    endif
  else
...
function s = bitwise_and(s)
  ...
  if state.int_mode
    ## INTEGER MODE: Mask according to bit width
    if state.int_bits == 8
      a_masked = bitand(uint64(a), uint64(255));
      b_masked = bitand(uint64(b), uint64(255));
      result = bitand(a_masked, b_masked);
    elseif state.int_bits == 16
      a_masked = bitand(uint64(a), uint64(65535));
      b_masked = bitand(uint64(b), uint64(65535));
      result = bitand(a_masked, b_masked);
    elseif state.int_bits == 32
      a_masked = bitand(uint64(a), uint64(4294967295));
      b_masked = bitand(uint64(b), uint64(4294967295));
      result = bitand(a_masked, b_masked);
    else  ## 64-bit
      result = bitand(uint64(a), uint64(b));
    endif
  else
    ## FLOATING-POINT MODE: Full 64-bit operation
    result = bitand(uint64(a), uint64(b));
  endif
  
  s=push(s, double(result));
  if state.debug
...
function s = bitwise_or(s)
  ...
  if state.int_mode
    ...
    if state.int_bits == 8
      a_masked = bitand(uint64(a), uint64(255));
      b_masked = bitand(uint64(b), uint64(255));
      result = bitor(a_masked, b_masked);
    elseif state.int_bits == 16
      a_masked = bitand(uint64(a), uint64(65535));
      b_masked = bitand(uint64(b), uint64(65535));
      result = bitor(a_masked, b_masked);
    elseif state.int_bits == 32
      ...
      result = bitor(uint64(a), uint64(b));
    endif
  else
    ## FLOATING-POINT MODE: Full 64-bit operation
    result = bitor(uint64(a), uint64(b));
...
function s = bitwise_xor(s)
  ...
  if state.int_mode
    ...
    if state.int_bits == 8
      a_masked = bitand(uint64(a), uint64(255));
      b_masked = bitand(uint64(b), uint64(255));
      result = bitxor(a_masked, b_masked);
    elseif state.int_bits == 16
      a_masked = bitand(uint64(a), uint64(65535));
      b_masked = bitand(uint64(b), uint64(65535));
      result = bitxor(a_masked, b_masked);
    elseif state.int_bits == 32
      ...
      result = bitxor(uint64(a), uint64(b));
    endif
  else
    ## FLOATING-POINT MODE: Full 64-bit operation
    result = bitxor(uint64(a), uint64(b));
...
function s = bitwise_not(s)
  ...
  if state.int_mode
    ...
    if state.int_bits == 8
      a_masked = bitand(uint64(a), uint64(255));
      result = bitxor(a_masked, uint64(255));
    elseif state.int_bits == 16
      a_masked = bitand(uint64(a), uint64(65535));
      result = bitxor(a_masked, uint64(65535));
    elseif state.int_bits == 32
      a_masked = bitand(uint64(a), uint64(4294967295));
      result = bitxor(a_masked, uint64(4294967295));
    else  ## 64-bit
      result = bitxor(uint64(a), uint64(18446744073709551615));
    endif
  else
    ## FLOATING-POINT MODE: Full 64-bit operation
    result = bitxor(uint64(a), uint64(18446744073709551615));
  ...
  s=push(s, double(result));
  if state.debug
    debug_after_op("~", sprintf("~%g = %g", a, double(result)), s);
  endif
endfunction
...
function s = shift_left(s)
  ...
  if state.int_mode
    ...
    if state.int_bits == 8
      a_masked = bitand(uint64(a), uint64(255));
      result = bitand(bitshift(a_masked, 1), uint64(255));
    elseif state.int_bits == 16
      a_masked = bitand(uint64(a), uint64(65535));
      result = bitand(bitshift(a_masked, 1), uint64(65535));
    elseif state.int_bits == 32
      a_masked = bitand(uint64(a), uint64(4294967295));
      result = bitand(bitshift(a_masked, 1), uint64(4294967295));
    else  ## 64-bit
      result = bitshift(uint64(a), 1);
    endif
  else
    ## FLOATING-POINT MODE: Full 64-bit operation
    result = bitshift(uint64(a), 1);
  ...
  s=push(s, double(result));
  if state.debug
    debug_after_op("{", sprintf("%g << 1 = %g", a, double(result)), s);
  endif
endfunction
...
function s = print_hex(s)
  ...
  if state.int_mode
    ## INTEGER MODE: Mask to current bit width
    if state.int_bits == 8
      if a < 0
        a_unsigned = bitand(int64(a), int64(255));
      else
        a_unsigned = mod(int64(a), 256);
      endif
      printf("%02X ", a_unsigned);
    elseif state.int_bits == 16
      if a < 0
    ...
    elseif state.int_bits == 32
      if a < 0
        a_unsigned = bitand(int64(a), int64(4294967295));
      else
        a_unsigned = mod(int64(a), 4294967296);
      endif
      printf("%08X ", a_unsigned);
    else  ## 64-bit
      ## Convert to unsigned int64
      if a < 0
        a_unsigned = uint64(int64(a));
      else
        a_unsigned = uint64(a);
      endif
      printf("%016X ", a_unsigned);
    endif
  else
    ## FLOATING-POINT MODE: Show full 64-bit representation
    if a < 0
      a_unsigned = uint64(int64(a));
    else
      a_unsigned = uint64(a);
...
function s = show_help(s)
  ...
  printf("  Hexadecimal:  #FF, #1F3A, #FFFF  (prefix with #)\n");
  printf("  Hex Display:  , operator shows bit-width-appropriate format\n");
  printf("                fp mode: 16 digits (64-bit), int8: 2 digits, int16: 4 digits\n");
  printf("                int32: 8 digits, int64: 16 digits\n");
  ...
  printf("I/O       | *  ,   | print number as hexadecimal (mode-aware)  | n --         | DONE\n");
  ...
  printf("FUNCTION  | * :_;  | define temporary block (auto-deleted)     | --           | DONE\n");
  ...
  printf("VARIABLE  | *  a-z | variable access (1-3 chars)               | -- n         | DONE\n");
  ...
  printf("  In FP mode:   All bitwise ops work on full 64-bit values (no masking)\n");
  printf("  In int8:      Operations masked to 8-bit (0-255)\n");
  printf("  In int16:     Operations masked to 16-bit (0-65535) - MINT 2 compatible\n");
  printf("  In int32:     Operations masked to 32-bit\n");
  printf("  In int64:     Operations work on full 64-bit values\n\n");
  printf("HEX DISPLAY:\n");
  printf("  FP mode:      , displays 16 hex digits (full 64-bit)\n");
  printf("  int8 mode:    , displays 2 hex digits (00-FF)\n");
  printf("  int16 mode:   , displays 4 hex digits (0000-FFFF)\n");
  printf("  int32 mode:   , displays 8 hex digits (00000000-FFFFFFFF)\n");
  printf("  int64 mode:   , displays 16 hex digits (full 64-bit)\n\n");
  printf("===================================================================================================\n");
  ...
  printf("Floating-point mode: fp (no bit-width limitations)\n");
  printf("===================================================================================================\n\n");
endfunction
...
function s = set_fp_mode(s)
  global state;
  state.int_mode = false;
  printf("\n*** FLOATING-POINT MODE ENABLED ***\n");
  printf("Division uses floating-point: 17/5 = 3.4\n");
  printf("Use for scientific calculations\n");
  printf("Bitwise ops work on full 64-bit values (no masking)\n\n");
endfunction

function s = show_mode(s)
  global state;
  printf("\n");
  if state.int_mode
    ...
    if state.int_bits == 8
      ...
      printf("  Bitwise ops masked to 8-bit\n");
      printf("  Hex display: 2 digits (00-FF)\n");
    elseif state.int_bits == 16
      printf("  Range: -32,768 to 32,767 (MINT 2 / TEC-1 compatible)\n");
      printf("  Bitwise ops masked to 16-bit\n");
      printf("  Hex display: 4 digits (0000-FFFF)\n");
    elseif state.int_bits == 32
      printf("  Range: -2,147,483,648 to 2,147,483,647\n");
      printf("  Bitwise ops masked to 32-bit\n");
      printf("  Hex display: 8 digits (00000000-FFFFFFFF)\n");
    else
      printf("  Range: ±9,223,372,036,854,775,807\n");
      printf("  Bitwise ops work on full 64-bit\n");
      printf("  Hex display: 16 digits (full 64-bit)\n");
    endif
...
function s = set_int8_mode(s)
  ...
  printf("Bitwise operations masked to 8-bit\n");
  printf("Hex display shows 2 digits (00-FF)\n\n");
endfunction
...
function s = set_int16_mode(s)
  ...
  printf("Bitwise operations masked to 16-bit\n");
  printf("Hex display shows 4 digits (0000-FFFF)\n\n");
endfunction
...
function s = set_int32_mode(s)
  ...
  printf("Bitwise operations masked to 32-bit\n");
  printf("Hex display shows 8 digits (00000000-FFFFFFFF)\n\n");
endfunction
